...
LuaEngine::LuaEngine(const std::filesystem::path& scriptPath)
    : m_lua(std::make_unique<sol::state>()),
      ...
      m_stats(
          std::make_unique<OffsetStatsGenerator>(scriptPath / "stats.csv")) {}
...
promise::Defer LuaEngine::load() {
  return newPromiseAsync(this, [this](Runner* caller, promise::Defer d) {
    ...
    auto result =
        lua().safe_script_file((m_scriptPath / "script.lua").string());
...
promise::Defer LuaEngine::readFromSim(uint32_t offset,
                                      uint32_t size,
                                      std::byte* data) {
      ...
      if (offsetTable.has_value()) {
        ...
        switch (type) {
          ...
          default:
            ...
            spdlog::error("Invalid datatype on read {}/{}: {}", offset, size,
                          type);
        }
...
promise::Defer LuaEngine::writeToSim(uint32_t offset,
                                     uint32_t size,
                                     const std::byte* data) {
  std::vector<std::byte> dataVec(data, data + size);

  return newPromiseAsync(this, [this, offset, dataVec](Runner* caller,
                                                       promise::Defer d) {
    sol::optional<sol::table> offsetTable = lua()["offsets"][offset];
    if (offsetTable.has_value()) {
      int type = offsetTable.value()[1];
      ...
      switch (type) {
        ...
        case 2:  // uint16
          result =
              writeFunc(*reinterpret_cast<const uint16_t*>(dataVec.data()));
          break;
        case 6:  // sint16
